...
| POST | `/channels/{id}/index-playlists` | **admin only** (`ADMIN_EMAILS`; 403 otherwise): asks TubeArchivist to index the channel's own playlists — the prerequisite for series feed sources. Sets the channel's `index_playlists` overwrite (instance-wide TA state) and queues TA's discovery task, which also stamps membership onto already-downloaded videos. The request **holds** until the task has run (up to ~40 s) and answers **200** `{ "status": "indexed", "playlists": PlaylistSummary[] }` — possibly empty, a channel with no playlists; **202** `{ "status": "pending" }` when TA is still at it, a big channel, to be followed on the GET below; **502** with TA's reason in `error` when the task failed. Web, iPhone and iPad hold on it and show what it found; the TV has no admin controls |
| GET | `/channels/{id}/index-playlists` | **admin only**: `{ "status": "running" \| "idle" }` — whether TubeArchivist is still running a playlist discovery, for a client that got `pending`. TA's stored task results name no channel, so this is "any discovery"; when none runs, the channel's playlists are whatever they are going to be |

### Videos
| Method | Path | Notes |
|---|---|---|
| GET | `/videos/{id}` | Video detail |
| GET | `/videos/{id}/up-next` | query `feed=<id>` or `playlist=<id>` or `channel=<id>`; **paged** VideoSummary of everything following the video in that context. Paged so a long playlist scrolls rather than being truncated. When **nothing follows** the answer is TA `similar` videos (minus any already watched or dismissed — neither is a suggestion) and the page carries **`"suggestions": true`**; the field is absent for a real queue. Clients must not autoplay into a suggested video, must not offer one as the end card's "up next", and must not show them under the context's own name — a guess presented as a queue is how "up next" stops meaning anything. `before=true` flips it: everything *preceding* the video, closest first, and empty (no fallback) when nothing does — the panel's "Previous" section |
...
| channel subscribe toggle (admin) | `POST /api/channel/` with `{ data: [{ channel_id, channel_subscribed }] }`. Subscribing queues TA's `subscribe_to` task and answers nothing to wait on, so `POST /channels` polls `GET /api/task/by-name/subscribe_to/` for a result it had not seen before the call — Celery's result meta, `result` an exception object on failure, `date_done` `false` until done — and then reads the channel. `POST /channels/{id}/index-playlists` waits on `by-name/index_playlists/` the same way |
