...
struct HistoryView: View {
    ...
    /// the hold menu just marked seen is a seen row (and its subtitle switches
    /// from a resume position to "seen …"), and one marked unseen has had its
    /// position cleared, which is what the server reports on the next load.
    /// `playlistId` and `feed` have to be carried over by hand — dropping them
    /// would leave the row playing with no context, so up next would answer
    /// with suggestions instead of the rest of the series.
